...
export default async (
     ...
     * @type {[type]}
     */
    const location = await $(elementSelector).getLocation(axis);
    ...
    if (falseCase) {
        expect(location).not.toEqual(
            intExpectedPosition,
            // @ts-expect-error
            `Element "${elementSelector}" should not be positioned at `
        ...
        expect(location).toEqual(
            intExpectedPosition,
            // @ts-expect-error
            `Element "${elementSelector}" should be positioned at `
