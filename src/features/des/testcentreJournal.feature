Feature: Test Centre Journal
    ...
    Scenario: Test centre Journal - Today Test
        ...
        Then I wait on element "test-center-journal::journal-button" to be displayed
        When I click on the element "test-center-journal::journal-button"
        Then I wait on element "test-center-journal::page-title" to be displayed
        Then I expect that element "test-center-journal::page-title" matches the text "Candidate search"
        And I expect that element "test-center-journal::location-text" matches the text "Example Test Centre"
        And I add "Ali Campbell" to the inputfield "test-center-journal::candidiate-search-text"
        And I click on the button with the text "Ali Campbell"
        And I wait on element "test-center-journal::show-booking-button" to be enabled
        And I click on the element "test-center-journal::show-booking-button"
        And I expect that element "test-center-journal::candidiate-booking-message" matches the text "This candidate does not have a booking at Example Test Centre for tomorrow."
