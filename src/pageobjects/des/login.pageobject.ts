...
class LoginPageObject extends Page {
  ...
  checkCookiesExist() {
    const cookies = driver.getAllCookies();
  ...
  loginWeb() {
    ...
    if (this.doesContextExistByArray([this._contextTitleForPage])) {
      ...
      this.msSubmitButton.click();
      this.msPasswordTextBox.waitForExist({
        timeout: 30000, reverse: false, interval: 50, timeoutMsg: `Element ${JSON.stringify(this.msPasswordTextBox)} did not exist on page within 15 seconds`,
      });
      this.msPasswordTextBox.setValue(superUser.Password);
      console.info('click sign in button');
      this.msSigninButton.click();
      ...
      this.msSigninButton.waitForExist({
        timeout: 30000, reverse: false, interval: 50, timeoutMsg: `Element ${JSON.stringify(this.msSigninButton)} did not exist on page within 15 seconds`,
      });

      // this.msContinueButton2.waitForExist({ reverse: true, interval: 50 });
      console.info('wait for continue button');
      this.msContinueButton2.waitForExist({
        timeout: 30000, reverse: false, interval: 50, timeoutMsg: `Element ${JSON.stringify(this.msContinueButton2)} did not exist on page within 15 seconds`,
      });

      this.msContinueButton2.waitForClickable({
        timeout: 30000, reverse: false, interval: 50, timeoutMsg: `Element ${JSON.stringify(this.msContinueButton2)} was not clickable within 15 seconds`,
      });
